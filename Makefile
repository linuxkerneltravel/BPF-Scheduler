...
all: all_targets # all是一个Makefile的默认目标，这里定义它依赖于all_targets，这意味着在运行make时会执行all_targets目标
...
# 定义了当前目录、工具目录、库目录、对象文件目录、可执行文件目录等路径
CURDIR := $(abspath .)
TOOLSDIR := $(abspath ..)
LIBDIR := $(TOOLSDIR)/lib
BPFDIR := $(LIBDIR)/bpf
...
# 这部分代码试图在多个位置寻找vmlinux文件，该文件是Linux内核的二进制镜像。如果找不到，会报错终止编译
...
# 最后这段代码检查是否存在生成的头文件autoconf.h，如果存在，定义GENFLAGS，添加编译标志-DHAVE_GENHDR，用于后续编译时判断是否生成了这些头文件
...
c-sched-targets = scx_simple scx_qmap scx_central scx_flatcg scx_me
